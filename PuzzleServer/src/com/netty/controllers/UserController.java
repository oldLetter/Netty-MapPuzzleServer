...
import com.netty.controllers.OtherResp.DefaultResp;
...
public class UserController {
	...
	public static class LogoutResp{
		@Tag(value=1)
		...
		if (MysqlOperation.getInstance().QueryOne("acount", req.usr)) {
			if (MysqlOperation.getInstance().QueryOne("pwd", req.pwd)) {
				if (ServerHandler.usermap.containsValue(req.usr)) {
					...
					ServerHandler.usermap.put(ctx.channel().id(), req.usr);
				}
	...
	public void Regist(byte[] byt, ChannelHandlerContext ctx) {
		...
		if (!MysqlOperation.getInstance().QueryOne("acount", req.acount)) {
			...
			ServerHandler.usermap.put(ctx.channel().id(), req.acount);
		} else {
			resp.result = RegistResp.ACOUNT_EXIST;
		}
		// ByteBuf buf=ProtoStuffSerializerUtil.serialize(resp,
		// RespDataID.RegistResp);
		ByteBuf buf = ProtoStuffSerializerUtil.serialize(resp);
		ctx.write(buf);
		ctx.flush();
	}
	...
	public void initData(byte[] byt, ChannelHandlerContext ctx) {
		...
		resp.userinfo = MysqlOperation.getInstance().QueryUserInfo("acount",
				ServerHandler.usermap.get(ctx.channel().id()).toString());
		resp.screlist = MysqlOperation.getInstance().QueryScorelist("acount",
				ServerHandler.usermap.get(ctx.channel().id()).toString());
		ByteBuf buf = ProtoStuffSerializerUtil.serialize(resp);
		ctx.writeAndFlush(buf);
	}
	public void ReconnectJoinUser(byte[] byt, ChannelHandlerContext ctx) {
		UserInfoReq req = ProtoStuffSerializerUtil.deserialize(byt, UserInfoReq.class);
		ServerHandler.usermap.put(ctx.channel().id(), req.acount);
	}
	public void Logout (byte[] byt, ChannelHandlerContext ctx) {
    	ServerHandler.usermap.remove(ctx.channel().id());
